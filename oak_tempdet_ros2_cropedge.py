...
class ImageSubscriber(Node):
    ...
    def __init__(self):
        ...
        fn_crop_left = 'img/yangmei_printed_crop_left.jpg'
        fn_crop_right = 'img/yangmei_printed_crop_right.jpg'
        ...
        self.templt_crop_left_img = cv.imread(cv.samples.findFile(fn_crop_left), cv.IMREAD_GRAYSCALE)
        self.templt_crop_right_img = cv.imread(cv.samples.findFile(fn_crop_right), cv.IMREAD_GRAYSCALE)
        ...
        self.kp_crop_left, self.desc_crop_left = self.detector_edge.detectAndCompute(self.templt_crop_left_img, None)
        self.kp_crop_right, self.desc_crop_right = self.detector_edge.detectAndCompute(self.templt_crop_right_img, None)
                ...
                # def match_and_draw(win):
                    ...
                    if self.detect_edge_value == 0:
                        ...
                        if self.detect_edge_value == 1:
                            explore_template_image = self.templt_crop_left_img
                            raw_matches = self.matcher.knnMatch(self.desc_crop_left, trainDescriptors = desc2, k = 2)
                            p1, p2, kp_pairs = filter_matches(self.kp_crop_left, kp2, raw_matches,ratio = 0.9)
                        elif self.detect_edge_value == 2:
                            explore_template_image = self.templt_crop_right_img
                            raw_matches = self.matcher.knnMatch(self.desc_crop_right, trainDescriptors = desc2, k = 2)
                            p1, p2, kp_pairs = filter_matches(self.kp_crop_right, kp2, raw_matches,ratio = 0.9)
                        elif self.detect_edge_value == 3:
